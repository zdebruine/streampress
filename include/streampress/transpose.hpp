...
///
/// Reads the file, decompresses, re-compresses with include_transpose=true,
/// and overwrites. Preserves value_type and row_sort settings from the header.
///
...
inline bool add_transpose(const std::string& path, bool verbose = true) {
    ...
    // Decompress (with reorder=true to get original row order)
    ...
    if (verbose) {
        SP_TRANSPOSE_MSG("[transpose] Done. Transpose added (%u chunks).\n",
                         stats.num_chunks);
    }
