...
#' @useDynLib streampress, .registration = TRUE
NULL
...
st_convert <- function(input, output, precision = "auto",
  ...
  x <- if (is.character(input)) {
    stop("File-to-file conversion not yet supported; read the file first.")
  } else if (inherits(input, "Matrix") || inherits(input, "matrix")) {
    input
  } else {
    stop("'input' must be a matrix, dgCMatrix, or a file path.")
  }
